...
function modificarAutor(urlAutor, id, options) {
  obtenerJson(urlAutor + id, options)
    .then((response) => {
      d.getElementById("nombre_" + id).innerHTML = response.nombre;
      d.querySelector(
        ".modal-body"
      ).innerHTML = `Autor: ${response.nombre} modificado`;
  ...
  if (e.target.matches(".crear")) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          `Se ha creado exitosamente el autor: <b>${nombre}</b>!`,
          "",
          "success"
        );
      } else {
  ...
  if (e.target.matches(".botonEstado")) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          `El estado del autor <b>${btn.dataset.nombre}</b> ha sido modificado a <b>${estadoFinal}</b>.`,
          "",
          "success"
        );
